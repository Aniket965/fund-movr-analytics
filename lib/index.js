...
export const handleData = async () => {
  ...
  const totaldata = mergeData([ethereumData,polygonData,binanceData,xdaiData]);
  const data = {
...
export const handleXdaiV0 = async () => {
  ...
  const data = await cachedFetch(
    `https://blockscout.com/xdai/mainnet/api?module=account&action=txlist&address=${registryAddress}`
  );
  let transactions = [];
  if (data.status === "1") {
...
export const getTokenPrice = async (tokenAddress, chainId) => {
  const data = await cachedFetch(
    `http://localhost:8000/v1/token-price?tokenAddress=${tokenAddress}&chainId=${chainId}
    `
  );
  return data.result;
}
