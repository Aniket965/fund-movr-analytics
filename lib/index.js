...
export const handlePolygonV1 = async () => {
  ...
  const chainId = 137
  const data = await cachedFetch(
    `https://api.polygonscan.com/api?module=account&action=txlist&address=${registryAddress}&sort=asc&apikey=YourApiKeyToken&endblock=25142921`,
  )
  const data_v2 = await cachedFetch(
    `https://api.polygonscan.com/api?module=account&action=txlist&address=${registryAddress}&sort=asc&apikey=CPZ9YW9NN8WZKSZD42T7ZY445UDH57PM1E&startblock=25142922`,
  )
  let transactions = []
  const data_final = data.result.concat(data_v2.result)
...
export const handleEthereumV1 = async () => {
  ...
  const chainId = 1
  const data = await cachedFetch(
    `https://api.etherscan.io/api?module=account&action=txlist&address=${registryAddress}&sort=asc&apikey=YourApiKeyToken`,
  )
  let transactions = []
  if (data.status === '1') {
    transactions = data.result.reduce((txs, tx) => {
