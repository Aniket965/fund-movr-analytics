...
import e1 from '../assets/ethereumv1.json'
import a1 from '../assets/arbitrumv1.json';
import o1 from '../assets/optimismv1p1.json';
import o2 from '../assets/optimismp2.json';
...
export const handleBinanceV1 = async () => {
  ...
  let transactions = []
  const datafinal = data_v2.result.concat(data_prefetched.result);
  if (data_v2.status === '1') {
    transactions = datafinal.reduce((txs, tx) => {
      try {
        const hash = tx.hash
        const input = tx.input
        txs.push({
...
export const handlePolygonV1 = async () => {
  const registryAddress = '0xc30141b657f4216252dc59af2e7cdb9d8792e1b0'
  const chainId = 137
  const p1 = await import('../assets/polygonv1part1.json')
  const p2 = await import('../assets/polygonv1part2.json');
  const p3 = await import('../assets/polygonv1part3.json')
  const p4 = await import('../assets/p4.json');

  const data = p1
  const data_v2 = p2;
  const data_v3 = p3;
  const data_v4 = p4
  const data_v5 = await cachedFetch(
    `https://api.polygonscan.com/api?module=account&action=txlist&address=${registryAddress}&sort=asc&apikey=CPZ9YW9NN8WZKSZD42T7ZY445UDH57PM1E&startblock=29204664`,
  )
  let transactions = []
  const data_final = data.result.concat(data_v2.result).concat(data_v3.result).concat(data_v4.result).concat(data_v5.result)
...
export const handleEthereumV1 = async () => {
  const registryAddress = '0xc30141b657f4216252dc59af2e7cdb9d8792e1b0'
  const chainId = 1
  const e1 = await import('../assets/ethereumv1.json')
  const prefeched_data = e1;

  const data = await cachedFetch(
    `https://api.etherscan.io/api?module=account&action=txlist&address=${registryAddress}&sort=asc&apikey=YM2ZSV4AVM947WN6R1ZYR2JT4X6B86Q8XJ&startBlock=14811950`,
  )

  const data_final = data.result.concat(prefeched_data.result)
  let transactions = []
  if (data.status === '1') {
    transactions = data_final.reduce((txs, tx) => {
      try {
...
export const handleOptimismV1 = async () => {
  const registryAddress = '0xc30141b657f4216252dc59af2e7cdb9d8792e1b0'
  const chainId = 10
  const prefetched_data = o1.result.concat(o2.result);


  const data = await cachedFetch(
    `https://api-optimistic.etherscan.io/api?module=account&action=txlist&address=${registryAddress}&sort=asc&apikey=YourApiKeyToken&startBlock=10771026`,
  )
  const finalData = data.result.concat(prefetched_data)
  let transactions = []
  if (data.status === '1') {
    transactions = finalData.reduce((txs, tx) => {
      try {
        const hash = tx.hash
        const input = tx.input
        txs.push({
...
export const handleArbitrV1 = async () => {
  const registryAddress = '0xc30141b657f4216252dc59af2e7cdb9d8792e1b0'
  const chainId = 42161
  const prefetched_data = a1;
  const data = await cachedFetch(
    `https://api.arbiscan.io/api?module=account&action=txlist&address=${registryAddress}&sort=asc&apikey=YourApiKeyToken&startBlock=12568876`,
  )
  const finalData = data.result.concat(prefetched_data.result)
  let transactions = []
  if (data.status === '1') {
    transactions = finalData.reduce((txs, tx) => {
      try {
...
export const handleEthereumV0 = async () => {
  ...
  const e0 = await import('../assets/ethereumv0.json');
  const chainId = 1
  const data = e0;
  let transactions = []
  if (data.status === '1') {
