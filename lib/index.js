...
export const handlePolygonV1 = async () => {
  ...
  const data = p1
  const data_v2 = p2;
  const data_v3 = await cachedFetch(
    `https://api.polygonscan.com/api?module=account&action=txlist&address=${registryAddress}&sort=asc&apikey=CPZ9YW9NN8WZKSZD42T7ZY445UDH57PM1E&startblock=27678302`,
  )
  console.log(data, data_v2,data_v3)
  let transactions = []
  const data_final = data.result.concat(data_v2.result).concat(data_v3.result)
